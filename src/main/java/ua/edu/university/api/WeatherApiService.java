    ...
    public JsonObject getYearlyMonthlyData(double lat, double lon) {
        ...
        try {
            ...
            for (int i = 0; i < times.size(); i++) {
                ...
                if (!tmax.get(i).isJsonNull()) {
                    tmaxSum[m] += tmax.get(i).getAsDouble();
                    cnt[m]++;
                }
                if (!tmin.get(i).isJsonNull()) {
                    tminSum[m] += tmin.get(i).getAsDouble();
                }
                if (!precip.get(i).isJsonNull()) {
                    precipSum[m] += precip.get(i).getAsDouble();
                }
            }
