...
### `PlotFactsBuilder` — заземлення AI на фактах

Формує блок зібраних даних (адреса, висота й перепад, відстані до дороги,
води та ЛЕП, санітарні чинники, клімат, POI), який додається до кожного
промту разом із забороною виходити за його межі.

Без нього модель отримувала **самі координати** і домислювала решту: ділянку
в с. Зубра описувала як «центральну частину Львова», а поруч із таблицею
«перепад висот 1.0 м» стверджувала про «виражений горбистий рельєф». Тепер
модель оперує тими самими числами, що й таблиці, і прямо зазначає, коли
даних для висновку бракує.
...
| 2 | Кліматичний і техніко-географічний паспорт: метеотаблиця, графік погоди, інфраструктура, адресні дані, методологія КП з підставами оцінок, транспортна доступність |
| далі | По одній сторінці на обраний шар (SCHEME → SLOPE): карта, експертний висновок Claude, тематична таблиця |
| остання | Підсумок: загальна придатність, сильні сторони, що потребує уваги, і одразу за ними геодезичний додаток — координати меж (WGS 84), до 50 точок |

Сторінки карт додаються лише для обраних шарів, тому обсяг змінний.
Наскрізний колонтитул із нумерацією малюється через `RunningFooter`
(`PdfPageEventHelper`) у полі під нижнім берегом, тому не зсуває контент.

Підсумкова сторінка формується **детерміновано** з порахованих
`SuitabilityCalculator.Factor`, а не через AI — інакше вона могла б
розійтися з рештою звіту.
...
**У вбудованому Arial немає гліфів зірочок** (U+2605/U+2606), тому рейтинги
у таблицях подаються словами: символьні рейтинги рендерились порожнечею.
